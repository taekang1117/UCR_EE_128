...
#define SYSTEM_CLOCK (48000000u)  // your board is effectively at 48 MHz
...
void init_uart1(uint32_t baudrate)
{
    ...
    UART1_C2 &= ~(UART_C2_TE_MASK | UART_C2_RE_MASK); // disable TX/RX while configuring
...
// Send 'L' multiple times to make sure UNO sees it
void send_lock_command(void)
{
    for (int i = 0; i < 8; i++) {
        UART1_PutChar('L');
    }
...
void init_gpio(void)
{
    ...
    GPIOD_PDDR |= 0xFF;         // PD0–PD7 outputs
...
{
    uint32_t reload = (SYSTEM_CLOCK / 1000u) - 1u;  // 1 ms tick
    SysTick->LOAD = reload;
    SysTick->VAL  = 0;
    ...
    while (1) {
        ...
        if (buttonFlag) {
            ...
            if (currentState == STATE_COUNTDOWN && seconds_left > 0) {
                ...
                send_lock_command();  // send multiple 'L's to UNO
