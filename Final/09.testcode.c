...
// We will REMOVE buttonFlag as the ISR will handle the logic
// volatile uint8_t  buttonFlag    = 0;
volatile uint8_t  timeoutFlag   = 0;
volatile uint8_t  countdown_active = 0;

// The current state must be volatile since it's accessed by both the main loop and the ISR
...
    // Use currentState for checking state
...
        // --- Immediate action *inside* the ISR ---
        // This ensures the transition is handled immediately upon the interrupt.
        if (currentState == STATE_COUNTDOWN && seconds_left > 0) {
            // Success: button pressed before timeout.
            // Change state and update internal variables
            currentState      = STATE_LOCKED;
            countdown_active  = 0;
            countdown_ms      = 0;
            seconds_left      = 9;
            displayFlag       = 1; // Flag for display update in main loop

            // Direct control of LEDs and Servo (fast GPIO/peripheral access)
            red_on();
            green_off();
            servo_set_locked();
            
...
// We must add 'buttonFlag' back to the global variables
volatile uint8_t  buttonFlag    = 0; 
// The other helper functions are unchanged for brevity:
// init_uart1, UART1_Available, UART1_GetChar, UART1_PutChar, 
// init_servo, servo_set_locked, servo_set_unlocked, 
// init_gpio, init_systick, main (partially)
...
        // --- 1) UART RX: any byte while LOCKED starts countdown ---
...
        // --- 2) Button success (UART TX triggered by ISR) ---
        // Note: The ISR now handles the state change, LED, and Servo.
        // We use buttonFlag only to indicate the slow UART TX is needed.
        if (buttonFlag) {
            buttonFlag = 0;
            UART1_PutChar('3');  // The slow I/O is done here in the main loop
        }
